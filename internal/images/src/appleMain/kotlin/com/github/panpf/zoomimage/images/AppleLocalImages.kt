...
class AppleLocalImages private constructor(cacheDir: Path) {

    companion object {

        private var instance: AppleLocalImages? = null
        private val lock = Mutex()

        suspend fun with(): AppleLocalImages {
...
//                        imageFiles = ComposeResImageFiles.values.toList(),
                    ...
                    AppleLocalImages(cacheDir).also { instance = it }
                }
            }
        }
    }
...
class AppleLocalImageFile(
    ...
    override fun toString(): String =
        "AppleLocalImageFile(name='$name', uri='$uri', size=$size, exifOrientation=$exifOrientation)"
}

fun ComposeResImageFile.toLocalImageFile(
    cacheDir: Path
): AppleLocalImageFile = AppleLocalImageFile(
    name = this.name,
