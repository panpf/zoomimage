...
class CoilModelToImageSourceImplTest {
    ...
    fun test() {
        ...
        try {
            ...
            assertEquals(
                expected = CoilHttpImageSource.Factory(context, imageLoader, httpUri),
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(httpUri))
            )
            assertEquals(
                expected = CoilHttpImageSource.Factory(context, imageLoader, httpUri),
            ...
            assertEquals(
                expected = CoilHttpImageSource.Factory(context, imageLoader, httpsUri),
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(httpsUri))
            )
            assertEquals(
                expected = CoilHttpImageSource.Factory(context, imageLoader, httpsUri),
            ...
            assertEquals(
                ...
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(contentUri))
            )
            assertEquals(
                expected = ContentImageSource(
                    context,
            ...
            assertEquals(
                expected = AssetImageSource(context, assetFileName).toFactory(),
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(assetUri))
            )
            assertEquals(
                expected = AssetImageSource(context, assetFileName).toFactory(),
            ...
            assertEquals(
                expected = FileImageSource(pathUri.toPath()).toFactory(),
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(pathUri))
            )
            assertEquals(
                expected = FileImageSource(pathUri.toPath()).toFactory(),
            ...
            assertEquals(
                expected = FileImageSource(fileUri.toUri().path!!.toPath()).toFactory(),
                actual = modelToImageSource.dataToImageSource(android.net.Uri.parse(fileUri))
            )
            assertEquals(
                expected = FileImageSource(fileUri.toUri().path!!.toPath()).toFactory(),
            ...
            assertEquals(
                expected = resourceId,
                actual = ((modelToImageSource.dataToImageSource(android.net.Uri.parse(resourceIntUri)) as WrapperFactory).imageSource as ResourceImageSource).resId
            )
            assertEquals(
                expected = resourceId,
