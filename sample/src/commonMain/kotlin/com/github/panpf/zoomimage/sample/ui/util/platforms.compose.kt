 ...
 */
@Stable
fun Size.isNotEmpty(): Boolean = width > 0f && height > 0f

/**
 * Round a [Size] down to the nearest [Int] coordinates.
 */
@Stable
fun Size.round(): IntSize =
 ...
 */
@Stable
fun Size.reverseRotate(rotation: Int): Size {
 ...
 */
@Stable
fun IntSize.toShortString(): String = "${width}x${height}"
 ...
 */
@Stable
fun IntSize.isNotEmpty(): Boolean = width > 0 && height > 0
 ...
 */
@Stable
fun IntSize.rotate(rotation: Int): IntSize {
 ...
 */
@Stable
fun IntSize.reverseRotate(rotation: Int): IntSize {
    val reverseRotation = (360 - rotation) % 360
    return rotate(reverseRotation)
}
 ...
 */
@Stable
fun IntSize.isSameAspectRatio(other: IntSize, delta: Float = 0f): Boolean {
 ...
 */
@Stable
fun IntSize.copy(width: Int = this.width, height: Int = this.height) =
 ...
 */
@Stable
fun Offset.rotateInSpace(spaceSize: Size, rotation: Int): Offset {
 ...
 */
@Stable
fun Offset.reverseRotateInSpace(spaceSize: Size, rotation: Int): Offset {
 ...
 */
@Stable
fun Offset.limitTo(rect: Rect): Offset {
 ...
 */
@Stable
fun IntOffset.toShortString(): String = "${x}x${y}"
 ...
 */
@Stable
fun IntOffset.rotateInSpace(spaceSize: IntSize, rotation: Int): IntOffset {
    require(rotation % 90 == 0) { "rotation must be a multiple of 90, rotation: $rotation" }
    return when ((rotation % 360).let { if (it < 0) 360 + it else it }) {
 ...
 */
@Stable
fun IntOffset.reverseRotateInSpace(spaceSize: IntSize, rotation: Int): IntOffset {
    val rotatedSpaceSize = spaceSize.rotate(rotation)
    val reverseRotation = (360 - rotation) % 360
    return rotateInSpace(rotatedSpaceSize, reverseRotation)
}
 ...
 */
@Stable
fun IntOffset.limitTo(rect: IntRect): IntOffset {
    return if (x < rect.left || x > rect.right || y < rect.top || y > rect.bottom) {
        IntOffset(
            x = x.coerceIn(rect.left, rect.right),
            y = y.coerceIn(rect.top, rect.bottom),
 ...
 */
@Stable
fun Rect.limitTo(rect: Rect): Rect =
 ...
 */
@Stable
fun Rect.limitTo(size: Size): Rect = limitTo(Rect(0f, 0f, size.width, size.height))
 ...
 */
@Stable
fun Rect.reverseRotateInSpace(spaceSize: Size, rotation: Int): Rect {
    val rotatedSpaceSize = spaceSize.rotate(rotation)
    val reverseRotation = (360 - rotation) % 360
    return rotateInSpace(rotatedSpaceSize, reverseRotation)
}
 ...
 */
@Stable
fun IntRect.toShortString(): String = "[${left}x${top},${right}x${bottom}]"

/**
 * Returns an IntRect scaled by multiplying [scale]
 */
@Stable
operator fun IntRect.times(scale: Float): IntRect =
    IntRect(
 ...
 */
@Stable
operator fun IntRect.times(scaleFactor: ScaleFactor): IntRect =
    IntRect(
 ...
 */
@Stable
operator fun IntRect.div(scale: Float): IntRect =
    IntRect(
        left = (left / scale).roundToInt(),
        top = (top / scale).roundToInt(),
        right = (right / scale).roundToInt(),
 ...
 */
@Stable
operator fun IntRect.div(scaleFactor: ScaleFactor): IntRect =
    IntRect(
        left = (left / scaleFactor.scaleX).roundToInt(),
        top = (top / scaleFactor.scaleY).roundToInt(),
        right = (right / scaleFactor.scaleX).roundToInt(),
 ...
 */
@Stable
fun IntRect.limitTo(rect: IntRect): IntRect =
    if (this.left < rect.left || this.left > rect.right
        || this.top < rect.top || this.top > rect.bottom
        || this.right < rect.left || this.right > rect.right
        || this.bottom < rect.top || this.bottom > rect.bottom
 ...
 */
@Stable
fun IntRect.rotateInSpace(spaceSize: IntSize, rotation: Int): IntRect {
    require(rotation % 90 == 0) { "rotation must be a multiple of 90, rotation: $rotation" }
    return when ((rotation % 360).let { if (it < 0) 360 + it else it }) {
        90 -> {
            IntRect(
 ...
 */
@Stable
fun IntRect.reverseRotateInSpace(spaceSize: IntSize, rotation: Int): IntRect {
    val rotatedSpaceSize = spaceSize.rotate(rotation)
    val reverseRotation = (360 - rotation) % 360
    return rotateInSpace(rotatedSpaceSize, reverseRotation)
}
 ...
 */
@Stable
fun ScaleFactor.toShortString(): String = "${scaleX.format(2)}x${scaleY.format(2)}"
 ...
 */
@Stable
val ScaleFactor.Companion.Origin: ScaleFactor
 ...
 */
@Stable
fun TransformOrigin.toShortString(): String =
 ...
 */
@Stable
operator fun Size.times(origin: TransformOrigin): Size =
    Size(
 ...
 */
@Stable
fun ContentScale.Companion.valueOf(name: String): ContentScale {
    return when (name) {
        "FillWidth" -> FillWidth
